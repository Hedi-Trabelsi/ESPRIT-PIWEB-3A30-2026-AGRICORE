...
class AnomalyService
{
    ...
    public function analyzeDepense(array $history, Depense $candidate): array
    {
        ...
        // Thresholds: z > 2.0 is often used for anomalies.
        // Also handle case where std is 0 (all values same) but candidate is much higher.
    ...
    /**
     * Analyze all expenses for a user to find all anomalies.
    ...
    public function analyzeAll(array $allDepenses): array
    {
        ...
        foreach ($allDepenses as $index => $candidate) {
            // History is everything EXCEPT the candidate for a fair check
            $history = $allDepenses;
            unset($history[$index]);
